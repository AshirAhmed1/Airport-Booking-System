...
# Helper Functions
def _duration_of_flight(dep: datetime.datetime, arr: datetime.datetime) \
        -> datetime.time:
    """
    Returns the total flight duration in hour, minute and seconds
    """
    duration = arr - dep
...
class FlightSegment:
    ...
    def __init__(
        ...
        self._flight_duration = _duration_of_flight(dep, arr)
    ...
    def book_seat(self, cid: int, seat_type: str) -> None:
        ...
        for i in range(len(self._manifest)):
            id_num = self._manifest[i][0]
            old_seat = self._manifest[i][1]
            if id_num == cid:
                customer_found = True
                if (old_seat != seat_type and seat_type in self.seat_capacity
                        and self.seat_availability[seat_type] > 0):
                    self._manifest[i] = (cid, seat_type)
                    self.seat_availability[old_seat] += 1
                    self.seat_availability[seat_type] -= 1

        if (not customer_found and seat_type in self.seat_capacity
                and self.seat_availability[seat_type] > 0):
    ...
    def cancel_seat(self, cid: int) -> None:
        ...
        for x in self._manifest:
            id_num = x[0]
            old_seat = x[1]
            if id_num == cid and old_seat:
                self._manifest.remove((id_num, old_seat))
                self.seat_availability[old_seat] += 1
                break
        return None
...
class Trip:
    ...
    def get_in_flight_time(self) -> int:
        ...
        """
        total_mins = 0
        for x in self._flights:
            j = x.get_duration()
            secs_conv = j.hour * 3600 + j.minute * 60 + j.second
            total_mins += secs_conv // 60
        return total_mins
    ...
    def get_total_trip_time(self) -> int:
        """ Returns the amount of time (in minutes) the trip is takes,
            ...
            flight on a layover).
        """
        first_dep = self._flights[0].get_times()[0]
        last_arr = self._flights[0].get_times()[1]
